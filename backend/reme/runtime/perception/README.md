...
跌倒运行时不再等待确定性 detector 先产生事件。最近的姿态窗口会按 250 ms 源时间步长连续评估，并结合中心下降、峰值运动、向下速度、躯干角变化和低位锚点确认。MIL 分数仍写入证据，但不能在缺少结构确认时单独升级为跌倒。第一次确认后事件锁存至少 10 秒，只有稳定站立或坐姿持续至少 5 秒才恢复，避免重叠窗口产生重复告警。
