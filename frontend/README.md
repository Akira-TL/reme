...
数字键 `1`–`4` 可快速切换四个场景。页面左下角的 `Debug` 按钮可展开原始运行时状态，直接查看 A 的 session、姿态分类/置信度/持续时间、动作转变，以及 B 的决策状态、来源、风险等级和完整 JSON。使用 `?debug=1` 可在打开页面时自动展开调试面板。

跌倒流程使用明确标注的演示编排，不宣称当前模型已经完成真实跌倒确诊；摄像头帧只在页面内存中处理，默认不录制。
