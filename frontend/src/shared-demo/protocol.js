...
function parseTypedMessage(value) {
  ...
  if (value.type === REME_DAY_REVISION_TYPE) {
    return hasExactKeys(value, [
      ...
      && value.dataset_id === "reme-aug-2026-demo-v1"
      && /^\d{4}-\d{2}-\d{2}$/.test(value.date)
      && isRevision(value.timeline_revision)
      && isRevision(value.summary_revision)
      && ["generating", "ready", "unavailable"].includes(value.summary_status)
