...
test("reduceAEvent 按 sequence 水位丢弃迟到与重复事件", () => {
  const feed = createFeedState();
  assert.ok(reduceAEvent(feed, envelope({ sequence: 5 }), SESSION));
  assert.equal(reduceAEvent(feed, envelope({ sequence: 5 }), SESSION), null);
  assert.equal(reduceAEvent(feed, envelope({ sequence: 4 }), SESSION), null);
