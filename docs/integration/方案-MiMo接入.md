...
- 历史缺口：现已归档到 `experiments/legacy_motion_demo/contracts.py` 的 `EventCandidate` 缺 `zone/pose_state/motion_level`，`EventType` 缺 normal 类，命名 `duration_ms` vs 合同 `duration`。该 spike 按 `CONTEXT.md` 属未接受探索，不应扩展为当前运行时合同。
