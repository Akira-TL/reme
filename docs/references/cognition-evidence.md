...
| `fall_confidence_min` | 0.59 | **按 A 的产出分布实测标定**：A 的 fall 分支置信度 = `clamp(0.55 + 0.12·min(drop/0.20−1,1) + 0.12·min(speed/0.65−1,1) + 0.08·r, 0, 0.95)`，其 fall 门限强制两个 min 项 ∈ [0,1]、帧准入门限强制 r ∈ [0.5,1.0]，故取值恒在 [0.59, 0.87]（0.95 上钳不可达）。0.59 即该分布的解析下界，非文献值；原值 0.7 落在区间内部会静默丢弃 [0.59,0.70) 的真跌倒 |
