...
在 `multi` 下手动进入跌倒场景只切换视觉展示，不会预授权麦克风或启动单人 MoveNet 跌倒规则。控制端必须显式说明这一限制，并提供独立的“切为单人并启用真实跌倒规则”动作；该动作本身是操作者明确启用安全演示链路，不得由画面候选数自动执行。若该动作需要等待单人模型惰性加载，必须绑定独立操作代次；期间发生任何场景/模式选择、停止、隐藏、采集代次或 controller 变化，都使旧 continuation 失效，禁止迟到结果把用户拉回跌倒场景或启动麦克风。
...
多人合同是滚动发布中的增量协议。新版 Relay 在既有、字段严格的 `relay_capabilities` 之外另发精确的 `pose_projection_capabilities`，声明 `anonymous-pose-batch/v1`；不得向旧能力消息加字段。新版控制端只有在同一 socket 明确收到该声明后才能开放 `multi` 并发送 batch/reset；连接超时后本代固定为不支持，迟到声明不能重新打开。单人帧保持可发，因此新版前端配旧 Relay 会安全锁定单人，旧前端配新版 Relay 则可忽略独立的新消息。

控制端的 `PUBLISHING` 与绿色实时点只代表当前本地帧已经在本连接按允许的合同成功调用发送；仅有摄像头 `live`、缓存的旧帧、模型就绪或 capability pending 都只能显示 waiting。连接代次变化、断线、模式切换、停止或隐藏会同步清除该发布回执。
...
模式切换必须使用采集代次防止旧异步结果回写。切换后先同步清除旧人物层；只有新代次的有效帧可重新显示。页面隐藏同步增加推理代次并发布 reset；恢复可见时重置新鲜度期限，但仍必须等同一个解码计数器严格增长后才恢复推理。停止采集、session/租约变化或断线仍按既有生命周期 fail close。浏览器若没有可靠的单调解码帧计数，或计数连续三秒不增长，人物层必须在有界时间内清除；不得使用裸 `currentTime`、交替计数 API 或重复媒体事件给最后一帧续命。异步加载、异步推理的 settlement 以及已经返回但失鲜的结果受三秒期限与 generation 校验约束；但是当前 MediaPipe `detectForVideo` 在主线程同步执行，若底层调用本身阻塞事件循环，JavaScript 定时器无法抢占。该同步阻塞、长任务和目标手机卡顿必须保留为未通过的设备 Gate；真正的硬时限需要把推理迁移到 Worker 或其他可中止的离主线程架构。多人推理可观察地超时/失败后原 estimator 必须作废，显式重试加载新实例；资源释放不得反向卡住摄像头停止、控制权释放或新一代采集。

controller 断线时必须先尝试撤销全部有效媒体 grant 并清空做饭识别证据，再处理仅用于展示的投影清场；投影清场异常不得跳过或回滚前述权威 fail-close。viewer 自身的新鲜度 watchdog 仍负责在清场消息无法送达时清除旧人物层。
