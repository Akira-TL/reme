...
- 确认后创建家庭心跳卡，并在控制端内存中保留一个短时刻草稿。
- 本人明确同意后才发送 `kitchen_moment` 媒体授权；拒绝、超时、切场景或 TTL 到期立即清理草稿并保持纯骨架。
...
- WebRTC 失败时告警保持有效，画面退回骨架并显示“告警已送达，视频连接失败”。
...
- 媒体 grant 只授予签发瞬间已连接的 viewer；晚加入者继续看骨架与事件状态。
...
6. **媒体 Gate**：厨房同意前和跌倒 `checking` 阶段抓包无视频轨道；告警后既有 viewer 收到真实轨道；授权到期后轨道停止。
7. **跨网 Gate**：同 Wi-Fi P2P 通过；不同网络如果只有 STUN 失败，必须显式降级，并保留 TURN/SFU 后续项，不能宣称普遍可用。
...
6. 完成手机/跨端 Gate 后只提交并推送 `upstream/lbx`。
