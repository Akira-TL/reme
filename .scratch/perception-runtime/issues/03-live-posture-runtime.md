...
- [ ] 复用冻结的姿态标签、特征和unknown阈值。
- [ ] 输出posture、confidence、duration、motion_level和landmark_quality。
- [ ] 姿态输出频率为5–10 Hz，不要求每帧分类。
- [ ] 人体不可用或证据不足时输出unknown，不沿用旧标签伪装稳定。
- [ ] 目标姿态到页面延迟P95不超过500 ms。
- [ ] 切换session后清空持续时长和分类缓存。
- [ ] 连续运行10分钟无阻断错误。
