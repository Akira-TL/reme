...
- 密钥只作为 Cloudflare Secret 的 SHA-256 摘要存在，不进入 Git、Vite 环境变量或浏览器存储。
- 正确密钥只签发短期不透明控制令牌；原始密钥成功解锁后立即从页面清除。为支持同一标签页刷新恢复，短期令牌、session、租约到期时间和场景仅写入该标签页的 `sessionStorage`，不使用 `localStorage`，关闭标签页或明确释放后失效。
- 同一 Demo 房间一次只允许一个未过期控制租约和一个控制 WebSocket。
- 第二个正确密钥请求也返回 `controller_locked`，不能抢占。
- 心跳续租；刷新或断线后在短期 TTL 内自动重连，Relay 返回权威事件/骨架序号以保持同一 session 递增；主动释放或超时后才允许下一位控制者。
