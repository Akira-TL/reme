...
_MODEL_PATH = Path("artifacts/pose-classification/fall-50/mil-v3/model.json")
_SAMPLES_PATH = Path("artifacts/pose-classification/fall-50/bootstrap/pose-samples.jsonl")


def _stub_model() -> FallMILModel:
    return cast(FallMILModel, SimpleNamespace(threshold=0.7))
...
@pytest.mark.skipif(
    not (_MODEL_PATH.is_file() and _SAMPLES_PATH.is_file()),
    reason="optional local MIL v3 model and bootstrap samples are not present",
)
